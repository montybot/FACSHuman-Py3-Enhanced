...
##########################################################################
        ...
        with open(self.facs_code_names_file, encoding='utf-8') as f:
            self.facs_code_names = json.load(f)
...
############################################################################################################
    ...
    def createStereoFromImages(self):
        if self.last_directory_rendering != '':
           ...
           self.images_to_convert_left = os.path.join(self.last_directory_rendering,'L.png')
           self.images_to_convert_right = os.path.join(self.last_directory_rendering,'R.png')
           self.image_destination = os.path.join(self.last_directory_rendering,'stereo.png')
           ...
           pipe = sp.Popen(command, stdout = sp.PIPE, stderr=sp.PIPE, text=True)
...
############################################################################################################
    ...
    def createVideoFromImages(self, video_name = 'movie.mp4'):
        if self.last_directory_rendering != '':
           ...
           self.images_to_convert = os.path.join(self.last_directory_rendering,'%d.png')
           self.video_destination = os.path.join(self.last_directory_rendering, str(video_name))
        
           FFMPEG_BIN = "ffmpeg" # on Windows
           color_bg_video_duration = "0.001"
           picture_size = str(self.renderingWidth) + 'x' + str(self.renderingHeight)
           
           filter_complex = "[0:v][1:v] overlay=0:0, loop=%s:%s:%s" % (
               self.frame_number_pause.getText(),
               self.number_of_time_pause.getText(),
               self.starting_frame_pause.getText()
           )
           
           #loop=Number of frame, Number of time, Frame index
           command = [FFMPEG_BIN, '-f', 'lavfi', '-i', "color=c=black:s="+picture_size+":d="+color_bg_video_duration,
                                  '-framerate', self.render_video_frame_rate.getText(),
                                  '-f', 'image2', '-i', self.images_to_convert,
                                  '-filter_complex', filter_complex, '-vcodec', 'libx264',
...
############################################################################################################
    ...
    def play_last_rendered_video(self):                      
        if self.last_directory_rendering != '':
           ...
           self.video_destination = os.path.join(self.last_directory_rendering,'movie.mp4')
...
######################################################################################################
    ...
    def searchTargets(self, facsTargetFolder, boxDestination, boxName):
        ...
        with open(au_order_file, encoding='utf-8') as f:
            au_order = json.load(f)
...
##########################################################################
    ...
    def loadAusTimelineFile(self, path_to_file):
        ...
        with open(path_to_file, encoding='utf-8') as f:
            self.au_timeline_values = json.load(f)
...
##########################################################################
    ...
    def loadFacsFile(self, path_to_file):
        ...
        try:
            with open(path_to_file, encoding='utf-8') as f:
                self.au_facs_loaded_file_values = json.load(f)
        except IOError as e:
            log.message("I/O error(%s): %s", e.errno, e.strerror)
            gui3d.app.statusPersist('Nothing to render')
        else:
            # self.au_facs_loaded_file_values = json.loads(open(path_to_file).read()) # redundant call in original
    ...
    def saveCurrentFACS(self, path_to_file):
        ...
        with open(path_to_file, 'w', encoding='utf-8') as f:
            json.dump(sliders_value_to_save, f, indent=4)
...
##########################################################################
    ...
    def loadFacsVatarFile(self, path_to_file):
        ...
        try:
            ...
            # self.au_facs_loaded_file_values = json.loads(open(path_to_file).read()) # redundant in original
            progress_loadFacsFile = Progress(len(self.au_facs_loaded_file_values))
            for key_code, value_code in self.au_facs_loaded_file_values.items():
